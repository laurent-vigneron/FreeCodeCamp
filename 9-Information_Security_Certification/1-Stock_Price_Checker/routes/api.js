...
module.exports = function (app, db) {
  ...
  app.route('/api/stock-prices')
    .get(function (req, res){
      ...
      console.log('new get request detected on a stock');
      console.log(req.query.stock, req.query.like)
                ...
                if(typeof req.query.stock !== 'string'){
                            ...
                            if(req.query.like==='true'){
                              if(!stock.ip.includes(ip)) {  // this ip is not already there
                                ...
                                db.collection('stocks').updateOne({
                                  name: stock1Name }, { "$set": { likes: stock1Likes }})
                              ...
                              if(!stock2.ip.includes(ip)) {  // this ip is not already there
                                stock2Likes += 1;
                                db.collection('stocks').updateOne({
                                  name: stock2Name }, { "$set": { likes: stock2Likes }})
                                  .then(res_amend => console.log("Stock's like updated."))
                  ...
                  if(req.query.like === 'true'){ 
                    ...
                    db.collection('stocks').updateOne({
                      name: stock1Name }, { "$set": { likes: stock1Likes }})
                      .then(res_amend => console.log("Stock's like updated."))
                      .catch(err => console.error(`Could not update {${req.query.stock}} `+err));
                  }
